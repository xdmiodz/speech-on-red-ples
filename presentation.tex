...
\author[ИПФ РАН]{М.~Е.~Гущин, С.~В.~Коробков, А.~В.~Костров, \underline{Д.~А.~Одзерихо}, С.~Э.~Привер, А.~В.~Стриковский}
...
\tableofcontents
\section{Введение}
\begin{frame}\frametitle{Роль НЧ волн в околоземной плазме}
  \small
  \begin{itemize}
...
\begin{frame}\frametitle{Параметры численного моделирования}
 \begin{columns}
   \small
   \begin{column}{6cm}      
     \begin{tabular}{|m{3cm}|l|}
       \hline
       Число частиц & $\approx{}10$\,млн.\\
       \hline
       Число циклов & $>100\,000$\\
       \hline
       Начальная температура частиц & 1 эВ\\
       \hline
       $f_{ce}$ & 65 МГц\\
       \hline
       $f_{m}$ & $400\div{}600$\,кГц\\
       \hline
       $E$ & $0.5\div{}3$\,В/см\\
       \hline
       Скважность импульсов & $2\div{}4$\\
       \hline
      \end{tabular}
     Использовалось одночастичное приближение, без учета коллективных явлений в плазме.
     %\vspace{4cm}
    \end{column}
    \begin{column}{5cm}
      \begin{figure}
        \centering
        \small
        \def\svgwidth{140pt} % sets the image width, this is optional
        \input{single_pulse.tex}
      \end{figure}
     \vspace{0.34cm}
    \end{column}
 \end{columns}
...
\begin{frame}\frametitle{Амплитудно-модулированная ВЧ накачка}
  \begin{columns}
   ...
   \begin{column}{6cm}
     \vspace{1cm}
      \begin{figure}
        ...
        \small
        \def\svgwidth{140pt} % sets the image width, this is optional
        \input{fm_600_khz.tex}
        \vspace{0.5cm}
        \caption{Возмущение поперечной энергии электронов $T$ как функция времени $t$ и продольного расстояния $z$ от <<антенны>>.}
      \end{figure}
     %\vspace{3cm}
      \small
    \end{column}
 \begin{column}{6cm}
   ...
   \begin{tabular}{|c|c|}
     \hline
     $\mathbf{f_m}$,\,кГц & $L/L_{эфф}$\\\hline
     400 & 2\\\hline
     600 & 2.5\\\hline
     800 & 5\\
     \hline
     \end{tabular}
   \caption{Зависимость отношения рассчетной длины $L=v_{Te}/f_{m}$ параметрической антенны к рассчитанной $L_{эфф}$ в ходе численного моделирования.}
   \end{table}
   \end{column}
\end{columns}
 \end{frame}
\begin{frame}\frametitle{ФРЭ по поперечной энергии}
  \begin{figure}
    \centering
    \small
    \def\svgwidth{170pt} % sets the image width, this is optional
    \input{df.tex}
    \vspace{0.5cm}
    \caption{Модификация функции распределения электронов (ФРЭ) по поперечной энергии под действием импульса ВЧ накачки. На графике не представлено образование <<хвоста>> функции распределения.}
  \end{figure}
\end{frame}
\section{Заключение}
\begin{frame}
\begin{itemize}
  \item На крупномасштабном плазменном стенде <<Крот>> проведены модельные лабораторные эксперименты, результаты которых свидетельствуют о возможности использования данной схемы параметрической генерации ОНЧ волн в активных ионосферных экспериментах, проводимых с использованием антенн, установленных на борту космических аппаратов, например --- ионозондов космического базирования.
    \item Используемая численная модель показала свою применимость для качественных и количественных рассчетов эффектов, возникающих в плазме под действием импульса ВЧ накачки в условиях ЭЦР. 
\end{itemize}
\end{frame}
\end{document}
