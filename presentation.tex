...
\section{Введение}
\begin{frame}\frametitle{Роль ОНЧ волн в ионосфере Земли}
  \small
  \begin{itemize}
...
\begin{frame}\frametitle{Методы генерации ОНЧ волн в околоземной плазме}
  \begin{itemize}
    \small
...
\begin{frame}\frametitle{Диамагнитный эффект}
  \small
\begin{itemize}
  \item  В условиях ЭЦР электроны увеличивают свою поперечную энергию и приобретают дополнительный магнитный момент, в плазме наблюдается диамагнитный эффект: уменьшение индукции внешнего магнитного поля в области, содержащей быстрые частицы.
  \item Если ВЧ сигнал, подводимый к антенне, модулирован по амплитуде с частотой $f_{m}$, то поток ускоренных электронов, и, соответственно, наведенный в возмущенной области плазмы магнитный момент изменяются с периодом модуляции.
\end{itemize}
 В результате, протяженная область пространства, в которую из ближней зоны передатчика поступает модулированный поток ускоренных по поперечной энергии электронов, может выступать в роли <<бестелесной>> антенны, излучающей волны на частоте модуляции в окружающую плазму.
\end{frame}

\begin{frame}\frametitle{Достоинства метода}
  \small
  \begin{itemize}
    \item Геометрические размеры «параметрической» антенны могут быть очень велики, что повышает эффективность генерации НЧ волн.
...
\begin{frame}\frametitle{Плазменный стенд <<Крот>>}
  ...
  \begin{tabular}{|l|l|}
    ...
    \multirow{4}{*}{Индукционный ВЧ источник плазмы} & f = 4 МГц\\
     ...
     & P = 1 МВт каждый\\
     & $t_p=1.5$\,мс\\\hline
    \multirow{3}{*}{Источник магнитного поля} & Емкостной акопитель\\
    & $E=1$\,МДж\\
...
\begin{frame}\frametitle{Схема экспериментов}
  \begin{columns}
    \begin{column}{5cm}
      \begin{itemize}
        \item Диагностика концентрации плазмы --- зонд с СВЧ-резонатором~\cite{Stenzel,Yanin}.
        \item Диагностика электронной температуры --- двойной зонд~\cite{UHF_probe}. 
      \end{itemize}
  \vspace{1cm}
      \begin{thebibliography}{9}
        \tiny
...
\begin{frame}\frametitle{Диамагнитный сигнал в плазме}
  \begin{columns}
    \begin{column}{5cm}      
      \small
      \begin{itemize}
      \item $(а)$ Импульс накачки мощностью $P\approx{}250$\,Вт, подаваемый на рамочную антенну
        \item $(б)$ неинтегрированный сигнал с магнитного зонда
        \item $(в)$ полное возмущение $\Delta{}B$ внешнего магнитного поля в условиях ЭЦР на расстоянии $z=3.5$\,см напротив витков антенны 
          \end{itemize}
       \vspace{0.3cm}
      \hspace{0.7cm}$n_{e}=10^{10}$\,см$^{-3}$, $B_{0}$ = 25$\,Гс$

      \hspace{0.7cm}$P\approx{}250$\,Вт
      \vspace{0.3cm}
...
\begin{frame}\frametitle{Диамагнитный эффект как функция $f/f_{ce}$ и мощности передатчика $P$}

  \begin{columns}
    \begin{column}{6cm}      
      \small
      \begin{itemize}
      \item  Основной резонанс ($f_{ce}$) несимметрично уширен в область низких частот, что связано с возбуждением антенной при $f<f_{ce}$ распространяющихся ВЧ волн свистового диапазона.
      \item Резонанс на второй гармонике ($2f_{ce}$) находится в области непрозрачности плазмы для электромагнитного излучения, $f_{ce}<f<f_{pe}$. Симметричное уширение этого резонанса обусловлено конечным временем пролета ускоряемых электронов через ближнюю зону антенны с невозмущенной продольной (тепловой) скоростью.
      \end{itemize}
    \end{column}
    \begin{column}{7cm}
      \begin{figure}
...
\begin{frame}\frametitle{Параметрическая генерация НЧ волн}
  \begin{columns}
    \begin{column}{6cm}      
      \small
      \begin{itemize}
        \item $(а)$ НЧ сигналы, регистрируемые при подаче на антенну амплитудно--модулированного с частотой $f_{m}$ импульса ВЧ накачки в условиях ЭЦР, при различном радиальном удалении $r$ магнитного зонда от антенны ($f_{m} = 400$\, кГц, $f=66.5$\,МГц $\simeq f_{ce}$, $P\simeq 300$\,Вт). 
          \item $(б)$ Зависимость поперечной скорости распространения $v_{\perp}$ НЧ возмущений внешнего магнитного поля от частоты модуляции $f_{m}$ импульса ВЧ накачки.
      \end{itemize}
      \vspace{0.5cm}
       \hspace{0.7cm}
       \begin{tabular}{|m{5cm}|}
         \hline
         Наблюдается возбуждение волн конической рефракции.\\
         \hline
         \end{tabular}
 \vspace{1.5cm}
    \end{column}
